...
class Paystack
{
     ...
     */
    public function startOneTimeTransaction($amount, $email, $tp_data, $plan = '')
    {
     ...
     * @throws \Exception|mixed|Exceptions\PaystackInvalidTransactionException
     */
    public function createTransferRecipient($accountName, $accountNumber, $bankCode, $currency = 'NGN', $type = 'nuban')
    {

        $transferRecipient = CreateTransferRecipient::make(
            $accountName,
            $accountNumber,
            $bankCode,
            $currency,
        ...
        $transferRecipient->setTransactionResource($this->getTransactionResource());
        $transaction =  $transferRecipient->createRecipient();
     ...
     * @throws \Exception|mixed|Exceptions\PaystackInvalidTransactionException
     */
    public function createRefund($transactionReference, $amount, $currency = 'NGN')
    {

        $refund = CreateRefund::make(
            $transactionReference,
            $amount,
            $currency
        );
        $refund->setTransactionResource($this->getTransactionResource());
     ...
     * @throws \Exception|mixed|Exceptions\PaystackInvalidTransactionException
     */
    public function createTransfer($amount, $recipient, $source = 'balance', $currency = 'NGN')
    {

        $transfer = CreateTransfer::make(
            $amount,
            $recipient,
            $source,
            $currency
        );
        $transfer->setTransactionResource($this->getTransactionResource());
     ...
     * @return \Exception|mixed|Exceptions\PaystackInvalidTransactionException
     */
    public function chargeReturningTransaction($authorization, $amount, $email, $metadata = [], $plan = '')
    {
    ...
    {
        return $this->getTransactionHelper()->banks();
    }
